...
func (m *MovieClip) filterByActionNameFrames(name string) []*Frame {
	frames := make([]*Frame, 0)
	for _, frame := range m.AllFrames() {
		if strings.Contains(frame.ResourceName, name) {
			frames = append(frames, frame)
		}
	}

	sort.Slice(frames, func(a, b int) bool {
		return frames[a].ResourceName > frames[b].ResourceName
	})
